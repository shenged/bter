module.exports = (function() {
	...
	return {
		...
		// Returns all tickers available
		getAllTickers: function(cb) { // no options
			if(!cb) throw new Error('No callback defined');
	
			throwback({method: 'GET', url: API_URL + 'tickers', headers: { 'User-Agent' : USER_AGENT } }, cb);
		},		
		...
		// Returns trade history in last 24 hours
		getTradeHistory: function(options, cb) { // options = API_KEY, SECRET_KEY
			if(!cb) throw new Error('No callback defined');
			if(!options) cb(new Error('No options defined'));
			if(!options.API_KEY || !options.SECRET_KEY) cb(new Error('You must defined both the public and private key'));

			var n = nonce(), 
				form = { 'pair': options.CURR_A + '_' + options.CURR_B,nonce: n },
				signed = signMessage(options.SECRET_KEY, 'pair=' + options.CURR_A + '_' + options.CURR_B + '&nonce=' + n);

			throwback({method: 'POST', url: API_URL + 'private/mytrades', form: form, headers: { 'User-Agent' : USER_AGENT, 'Key': options.API_KEY, 'Sign': signed } }, cb);
		}		
	};
})();
